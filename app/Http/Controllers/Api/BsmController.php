...
// Buat paginasi manual pada koleksi lembagaList
    ...
    public function store(Request $request)
    {
        try {
            ...
            $existingRecord = Bsm::where('tahun_id', $request->tahun_id)
     ...
     */
    public function update(Request $request, Bsm $bsm)
    {
    }

    public function updateFile(Request $request, Bsm $bsm)
    {
        try {
            ...
            if ($bsm->path && Storage::disk('public')->exists($bsm->path)) {
                Storage::disk('public')->delete($bsm->path);
            }
     ...
     */
    public function destroy(Bsm $bsm)
    {
        try {
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'File Laporan berhasil dihapus.',
            ]);
    ...
    public function download(Bsm $bsm)
    {
        try {
            // 1. Cari data laporan berdasarkan ID
            $filePath = $bsm->path;
            $fileName = $bsm->name_file;
