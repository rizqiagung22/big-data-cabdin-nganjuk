...
use App\Models\PendistribusianIjazah;
...
class PendistribusianIjazahController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $query = PendistribusianIjazah::with(['tahun'])
...
// Buat paginasi manual pada koleksi lembagaList
    ...
    public function store(Request $request)
    {
        try {
            ...
            $existingRecord = PendistribusianIjazah::where('tahun_id', $request->tahun_id)
            ...
            $path = $file->store('pendistribusian-ijazah', 'public');

            // 4. Simpan data ke database dalam transaksi
            DB::beginTransaction();
            $laporan = PendistribusianIjazah::create([
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'BSM berhasil diunggah dan disimpan.',
                'data' => $laporan,
            ], 201); // 201 Created
     ...
     */
    public function show(PendistribusianIjazah $pendistribusianIjazah)
    {
        //
    }
     ...
     */
    public function update(Request $request, PendistribusianIjazah $pendistribusianIjazah)
    {
    }

    public function updateFile(Request $request, PendistribusianIjazah $pendistribusianIjazah)
    {
        try {
            ...
            if ($pendistribusianIjazah->path && Storage::disk('public')->exists($pendistribusianIjazah->path)) {
                Storage::disk('public')->delete($pendistribusianIjazah->path);
            }
     ...
     */
    public function destroy(PendistribusianIjazah $pendistribusianIjazah)
    {
        try {
    ...
    public function download(PendistribusianIjazah $pendistribusianIjazah)
    {
        try {
            // 1. Cari data laporan berdasarkan ID
            $filePath = $pendistribusianIjazah->path;
            $fileName = $pendistribusianIjazah->name_file;
