...
class BosController extends Controller
{
    ...
    public function updateFile(Request $request, Bos $bos)
    {
        try {
            ...
            if ($bos->path && Storage::disk('public')->exists($bos->path)) {
                Storage::disk('public')->delete($bos->path);
            }
     ...
     */
    public function destroy(Bos $bos)
    {
        try {
    ...
    public function download(Bos $bos)
    {
        try {
            // 1. Cari data laporan berdasarkan ID
            $filePath = $bos->path;
            $fileName = $bos->name_file;
